...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedAssetResult> mergedAsset(List<String> var1,
    ...
    return customSelect(
        'SELECT * FROM (SELECT DISTINCT rae.id AS remote_id, IIF(la.backup_selection = 0, lae.id, NULL) AS local_id, rae.name, rae.type, rae.created_at, rae.updated_at, rae.width, rae.height, rae.duration_in_seconds, rae.is_favorite, rae.thumb_hash, rae.checksum, rae.owner_id, rae.live_photo_video_id, 0 AS orientation, rae.stack_id, COALESCE(stack_count.total_count, 0) AS stack_count FROM remote_asset_entity AS rae LEFT JOIN local_asset_entity AS lae ON rae.checksum = lae.checksum LEFT JOIN local_album_asset_entity AS laa ON laa.asset_id = lae.id LEFT JOIN local_album_entity AS la ON la.id = laa.album_id LEFT JOIN stack_entity AS se ON rae.stack_id = se.id LEFT JOIN (SELECT stack_id, COUNT(*) AS total_count FROM remote_asset_entity WHERE deleted_at IS NULL AND visibility = 0 AND stack_id IS NOT NULL GROUP BY stack_id) AS stack_count ON rae.stack_id = stack_count.stack_id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandedvar1) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT DISTINCT NULL AS remote_id, lae.id AS local_id, lae.name, lae.type, lae.created_at, lae.updated_at, lae.width, lae.height, lae.duration_in_seconds, lae.is_favorite, NULL AS thumb_hash, lae.checksum, NULL AS owner_id, NULL AS live_photo_video_id, lae.orientation, NULL AS stack_id, 0 AS stack_count FROM local_asset_entity AS lae LEFT JOIN remote_asset_entity AS rae ON rae.checksum = lae.checksum LEFT JOIN local_album_asset_entity AS laa ON laa.asset_id = lae.id LEFT JOIN local_album_entity AS la ON la.id = laa.album_id WHERE rae.id IS NULL AND la.backup_selection = 0) ORDER BY created_at DESC ${generatedlimit.sql}',
        variables: [
  ...
  i5.$LocalAlbumAssetEntityTable get localAlbumAssetEntity =>
      i1.ReadDatabaseContainer(attachedDatabase)
          .resultSet<i5.$LocalAlbumAssetEntityTable>(
              'local_album_asset_entity');
  i6.$LocalAlbumEntityTable get localAlbumEntity =>
      i1.ReadDatabaseContainer(attachedDatabase)
          .resultSet<i6.$LocalAlbumEntityTable>('local_album_entity');
  i7.$StackEntityTable get stackEntity =>
      i1.ReadDatabaseContainer(attachedDatabase)
          .resultSet<i7.$StackEntityTable>('stack_entity');
}
